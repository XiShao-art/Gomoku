...
class AI(object):
    ...
    def go(self, chessboard):
      #  print((np.where(chessboard==0)))
       ...
       # print('asdasd')
        ...
        if    chessboard[8,8]==COLOR_NONE and list1[0]==4 and list2[0]==4:
            new_pos[0]=8
            new_pos[1]=8
        ...
        for i in range(0 ,15):
            ...
            for j in range(0,15):
                ...
                if chessboard[i,j]==COLOR_NONE:
                    ...
                    temp2=self.circul (i, j, self.new_chessboard(chessboard),color,1)
                    if temp2 !=-1:
                        temp=temp2[0]
                        thePoint[0]= temp2[1]
                        thePoint[1] = temp2[2]
    ...
    def circul (self, i, j, chessboard,color,count):
        if count==0:
            return -1
        new = self.new_chessboard(chessboard)
        new[i, j] = color
        list1 = self.the_max_grades1(-color, new)
        list2 = self.the_max_grades1(color, new)
        new_pos123 = [0, 0]
        if list1[0] >= list2[0]:
        ...
        new[new_pos123[0], new_pos123[1]] = -color

        for i1 in range(0, 15):

            for j1 in range(0, 15):

                if new[i1, j1] == COLOR_NONE:
                    # print(i)

                    temp2 = self.count_the_total_grades(i1, j1, new, color)
                    if temp2 >= 50000:

                        #  print( i, j, new_pos123, i1, j1, temp2, color)
                        return temp2, i1, j1
                    elif temp2>=110:
                        self.circul( i1, j1, new, color, count-1)
    ...
    def new_chessboard(self, chessboard):
        new=np.zeros((self.chessboard_size, self.chessboard_size), dtype=np.int)
        for i in range(0,15):
            for j in range(0,15):
                new[i][j]=chessboard[i][j]
        return new
